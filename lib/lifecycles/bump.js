...
const fetchVersionHandlers = require('../handlers/fetchVersionHandlers')
const writeHandlers = require('../handlers/writeHandlers')
...
function fetchOldVersion (args, configPath) {
  var handlers = fetchVersionHandlers(configPath)
  return handlers[args.lang]()
}

function writeToConfig (args, newVersion, configPath) {
  var handlers = writeHandlers(args, newVersion, configPath)
  return handlers[args.lang]()
}

/**
 * attempt to update the version # in a collection of common config
 * files according to the language
 *
  ...
  Object.keys(configsToUpdate).forEach(function (configPath) {
    try {
      ...
      if (stat.isFile()) {
        ...
        var oldVersion = fetchOldVersion(args, configPath)
        checkpoint(args, 'bumping version in ' + filename + ' from %s to %s', [oldVersion, newVersion])
        writeToConfig(args, newVersion, configPath)
