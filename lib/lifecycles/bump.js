...
function bumpVersion (args, callback) {
  return new Promise((resolve, reject) => {
    if (args.releaseAs) {
      return resolve({
        releaseType: args.releaseAs
      })
    } else {
...
function updateConfigs (args, newVersion, pkg) {
  ...
  pkg.versionFiles.forEach((filename) => {
  ...
  Object.keys(configsToUpdate).forEach(function (configPath) {
    try {
      ...
      if (stat.isFile()) {
        ...
        checkpoint(args, 'bumping version in ' + filename + ' from %s to %s', [pkg.version, newVersion])
        if (!args.dryRun) {
          pkg.handler.updateVersion(newVersion, configPath)
        }
